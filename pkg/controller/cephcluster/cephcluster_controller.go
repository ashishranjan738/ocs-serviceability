...
// blank assignment to verify that ReconcileCephCluster implements reconcile.Reconciler
...
func (r *ReconcileCephCluster) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	if instance.Status.CephStatus != nil || instance.Status.CephStatus.Health != "HEALTH_OK" {
		cmd := exec.Command("gather_rook_ceph")
		cmd.Stdout = os.Stdout
		cmd.Stderr = os.Stderr
		err := cmd.Run()
		if err != nil {
			reqLogger.Error(err, "execution failed")
		}
	}
